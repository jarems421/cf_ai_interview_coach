...
const SESSION_TYPE_CONTEXT: Record<SessionType, string> = {
  quick_practice:
    "Session mode: Quick Practice - one question at a time with instant feedback.",
  full_mock:
    "Session mode: Full Mock Interview - simulate a complete 5-8 question interview, then produce a final report.",
  project_defence:
    "Session mode: Project Defence - the candidate has chosen a specific project; ask deep, probing questions about it (design decisions, tradeoffs, failures, improvements).",
  technical_screen:
    "Session mode: Technical Screen - ask practical role-calibrated technical questions. Prefer scenarios that test reasoning, constraints, debugging, systems behavior, tradeoffs, and edge cases over trivia.",
  company_specific:
    "Session mode: Company-specific - tailor all questions to the company culture, tech stack, and role expectations."
};

const INTERVIEW_MODE_INSTRUCTIONS: Record<InterviewMode, string> = {
  behavioural:
    "Focus on STAR-format behavioural questions (Situation, Task, Action, Result). Probe for real examples.",
  technical:
    "Focus on technical accuracy and engineering judgment. Ask scenario-based questions calibrated to the role and level; expect the candidate to reason through constraints, implementation choices, failure modes, edge cases, and tradeoffs.",
  project_deep_dive:
    "Pick one significant project from the candidate's background and systematically probe it: motivation, design, implementation, results, and what they would change.",
  company_motivation:
    "Ask why the candidate wants this specific company and role. Probe for genuine knowledge of the company's mission, products, and challenges.",
  weakness_gap:
    "Probe for honest self-assessment: weaknesses, gaps in experience, failures, and growth areas. Look for self-awareness and a growth mindset.",
  final_simulation:
